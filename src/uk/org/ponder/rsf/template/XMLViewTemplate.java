...
public class XMLViewTemplate implements ViewTemplate {
  public int INITIAL_LUMP_SIZE = 100;
  XMLLump rootlump;
  XMLLumpMMap globalmap;
  ...
  private void processTagStart(XmlPullParser parser, boolean isempty) {
    ...
    headlump.text = XMLLump.tagToText(tagname);
    ...
    for (int i = 0; i < attrs; ++i) {
      ...
      frontlump.text = lumpac.toString();
    ...
    finallump.text = attrs == 0? (isempty? "/>" : ">"):(isempty? "\"/>" : "\">");
  ...
  public ViewRender getViewRender() {
    ViewRender render = new ViewRender();
    render.setTemplate(this);
    return render;
  }
